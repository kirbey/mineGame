...
var score = 0;
function initGame() {
    game = Array(size * size).fill(null); initBestScore()
}
...
function drawBackground() {
    ...
    for (var i = 0; i < game.length; i++) {
        ...
        tileDiv.style.top = y * 100 + "px"; tileDiv.style.left = x * 100 + "px";
        tileDiv.classList.add("background");
        tileContainer.appendChild(tileDiv)
    }
}
function positionTile(tile, elm) {
    var x = tile.index % size; var y = Math.floor(tile.index / size);
    elm.style.top = y * 100 + "px"; elm.style.left = x * 100 + "px"
}
        ...
        if (tile) {
            if (isNew) {
                (function () {
                    ...
                    tileDiv.innerHTML = "<p>" + tile.value + "</p>"; tileContainer.appendChild(tileDiv)
                })()
            } else {
...
function gameOver() {
    if (game.filter(function (number) {
        ...
        var sameNeighbors = game.find(function (tile, i) {
            var isRightSame = game[i + 1] && (i + 1) % 4 !== 0 ? tile.value === game[i + 1].value : false; var isDownSame = game[i + 4] ? tile.value === game[i + 4].value : false;
...
function generateNewNumber() {
    var p = Math.random() * 100; return p <= 90 ? 2 : 4
}
function addRandomNumber() {
    ...
    var newObj = {
        id: nextId++, index: newPos, value: generateNewNumber()
    };
...
function rotateRight90Deg(grid) {
    var rotatedGame = Array(size * size).fill(0); for (var row = 0; row < size; row++) {
        for (var col = 0; col < size; col++) {
            var index1 = getIndexForPoint(col, row); var index2 = getIndexForPoint(row, size - 1 - col); rotatedGame[index1] = grid[index2]
    ...
    for (var i = 0; i < size; i++) {
        ...
        for (var _iterator3 = filteredRow, _isArray3 = Array.isArray(_iterator3), _i3 = 0, _iterator3 = _isArray3 ? _iterator3 : _iterator3[Symbol.iterator](); ;) {
            ...
            if (filteredRow[j].value === filteredRow[j + 1].value) {
                var sum = filteredRow[j].value * 2; filteredRow[j] = {
                    id: nextId++, mergedIds: [filteredRow[j].id, filteredRow[j + 1].id], value: sum
                };
                filteredRow.splice(j + 1, 1); score += sum; totalAdd += sum
            }
        ...
        scoreDiv.innerHTML = score; addDiv.innerHTML = "+" + totalAdd;
        addDiv.classList.add("active");
        setTimeout(function () {
            addDiv.classList.remove("active")
        }, 800);
        if (score > bestScore) {
            localStorage.setItem("bestScore", score); initBestScore()
        }
    }
    return newGameState
}
...
function handleTouchStart(evt) {
    xDown = evt.touches[0].clientX; yDown = evt.touches[0].clientY
}
function handleTouchMove(evt) {
    ...
    game = game.map(function (tile, index) {
        if (tile) {
            return _extends({}, tile, { index: index })
        } else {
            return null
        }
    });
...
function handleKeypress(evt) {
    ...
    if (!modifiers) {
        ...
        addRandomNumber(); updateDOM(prevGame, game);
        if (gameOver()) {
            setTimeout(function () {
                endDiv.classList.add("active")
            }, 800);
